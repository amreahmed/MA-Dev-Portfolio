...
export const BentoGrid = ({ className, children }: { className?: string; children?: React.ReactNode }) => {
  return (
    <div className={cn("grid grid-cols-1 md:grid-cols-6 lg:grid-cols-5  lg:gap-8 gap-4 max-w-7xl mx-auto ", className)}>
      {children}
    </div>
  );
};
...
export const BentoGridItem = ({
  ...
  const handelCopy = () => {
    navigator.clipboard.writeText("amr43103@gmail.com");
    setCopied(true);
  };
  return (
    <div
          ...
          {id === 3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              ...
              <div className="flex flex-col gap-3 lg:gap-6">
                {["Next.js", "React.js", "Node.js"].map((item, i) => (
                  <span
                    key={i}
              ...
              <div className="flex flex-col gap-3 lg:gap-5">
                <span className="py-4 px-3 rounded-lg text-center bg-[#10132E]"></span>
                {["MongoDB", "Discord.js", "Express"].map((item, i) => (
                  <span
                    key={i}
                    className="py-2 lg:py4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132e]">
                    {item}
